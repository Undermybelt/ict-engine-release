...
class FactorCandidateResolverTests(unittest.TestCase):
    def test_build_candidate_registry_marks_invalid_freqtrade_zip_unbuildable(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(candidate["evidence_status"], "board_evidence_only")
    ...
    def test_build_candidate_registry_without_profile_stays_generic(self) -> None:
        ...
        self.assertTrue(
            all(candidate["artifact_ready"] is False for candidate in bundle["candidates"])
        )
        deferred = next(
            item
    ...
    def test_build_candidate_registry_with_profile_marks_reusable_artifacts(self) -> None:
        ...
        self.assertEqual(killzone["artifact_kind"], "freqtrade_backtest_zip")
        self.assertEqual(killzone["board_evidence_status"], "board_recorded")
        self.assertIn("candidate_pack", killzone["naming_contract"]["artifact_layers"])
        self.assertGreaterEqual(len(killzone["reusable_input_refs"]), 2)
        self.assertTrue(any(ref.endswith(".zip") for ref in killzone["reusable_input_refs"]))
        displacement = next(
            item
            for item in bundle["candidates"]
        ...
        self.assertEqual(displacement["artifact_kind"], "freqtrade_backtest_zip")
        fvg_retrace = next(
            item
            for item in bundle["candidates"]
        ...
        self.assertTrue(eur_breakout["artifact_ready"])
