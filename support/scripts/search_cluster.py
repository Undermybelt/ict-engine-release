#!/usr/bin/env python3
"""Public cluster-search entrypoint."""
import argparse
import os
import pathlib
import subprocess
...
TARGET = REPO / 'support' / 'scripts' / 'archive' / 'factor_cluster_jump_v2.py'

BACKEND_HELP = """Archived cluster-jump backend. It can launch multiple autoresearch sessions in parallel and may take minutes.

Backend: support/scripts/archive/factor_cluster_jump_v2.py
...
EPILOG = """
When to use:
  - Use when local search is exhausted and you need a larger cluster jump.
  - Best for testing whether a new factor family beats the current baseline.

Examples:
  python3 support/scripts/search_cluster.py
  python3 support/scripts/search_cluster.py --target
  python3 support/scripts/search_cluster.py --backend-help
  python3 support/scripts/search_cluster.py --show-config
  python3 support/scripts/search_cluster.py --run --data-root /path/to/ict-cleaned-mtf
  python3 support/scripts/search_cluster.py --run
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description='Run or inspect the archived cluster-jump experiment entrypoint.',
        epilog=EPILOG,
