...
def resolve_binary_path(anchor: str | Path) -> Path:
    ...
    if override is not None:
        return override
    repo = resolve_repo_root(anchor)
    target_dir = _path_from_env("CARGO_TARGET_DIR") or repo / ".local-artifacts" / "cargo-target"
    release_bin = target_dir / "release" / "ict-engine"
    debug_bin = target_dir / "debug" / "ict-engine"
