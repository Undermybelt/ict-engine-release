...
### Existing helper: `support/scripts/archive/pre_bayes_policy_tuning.py`
- 每个 run 独立 state_dir
...
### Proposed helper: `bridge_gap_param_sweep.py`
- 扫 5 个 bridge-sensitive params
- 每个 run isolated
- 输出 bridge_gap 排名表

### Proposed helper: `cross_market_smt_bridge_experiment.py`
- base_factor=`cross_market_smt`
- 扫 lookback
- 比较 bridge_gap / gate / score

### Proposed helper: `evidence_bridge_summary.py`
- 汇总三条路径的最佳结果
- 输出最优方向
...
建议用 `/background` 跑：
- `python3 support/scripts/archive/pre_bayes_policy_tuning.py`
- proposed helper `bridge_gap_param_sweep.py`
- proposed helper `cross_market_smt_bridge_experiment.py`
- proposed helper `evidence_bridge_summary.py`
