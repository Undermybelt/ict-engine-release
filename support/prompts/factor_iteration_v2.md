...
```bash
python3 support/scripts/archive/factor_local_search_v2.py
```
...
./target/release/ict-engine factor-autoresearch \
  --symbol NQ \
  --data <cleaned-mtf-root>/cleaned-15m/nq.continuous-15m.json \
  --data-1m <cleaned-mtf-root>/cleaned-1m/nq.continuous-1m.json \
  --data-5m <cleaned-mtf-root>/cleaned-5m/nq.continuous-5m.json \
  --data-15m <cleaned-mtf-root>/cleaned-15m/nq.continuous-15m.json \
  --data-1h <cleaned-mtf-root>/cleaned-1h/nq.continuous-1h.json \
  --data-4h <cleaned-mtf-root>/cleaned-4h/nq.continuous-4h.json \
  --data-1d <cleaned-mtf-root>/cleaned-1d/nq.continuous-1d.json \
  --objective expansion_manipulation \
...
```bash
python3 support/scripts/archive/factor_iteration_v2_summary.py
```
