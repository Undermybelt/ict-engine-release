...
> Authoritative execution board for translating `support/docs/plans/202605081903searched` into repo-bounded workstreams.
...
**Tech Stack:** Rust CLI public surfaces (`provider-status`, `workflow-status`, `factor-research`, `factor-autoresearch-status`, `policy-training-status`, `artifact-status`, `export-structural-path-ranking-target`, `auto-quant-results-import`, `auto-quant-prior-init`, `auto-quant-ingest-real-trades`), repo docs under `support/docs/`, additive external helpers under `support/scripts/research/` and `support/scripts/auto_quant_external/`, explicit `/tmp/...` state dirs, JSON/JSONL/CSV artifact surfaces, optional research-side Parquet companions only when needed.

**Baseline / Authority Refs:** `support/docs/plans/202605081903searched`, `support/docs/audits/2026-05-08-paper-search-outline-and-matrix.md`, `support/docs/structural-belief-learning-repo-map.md`, `support/docs/plans/2026-05-05-algo20250505-ict-engine-integration-plan.md`, `support/docs/plans/2026-05-05-execution-tree-factor-auto-quant-todo.md`, `support/docs/plans/2026-05-07-auto-quant-post-factor-runtime-closure-todo.md`, `support/docs/plans/2026-05-02-catboost-path-ranking-target-design.md`, `src/application/provider_catalog.rs`, `src/market_catalog/mod.rs`, `src/application/regime/persistence.rs`, `src/application/orchestration/structural_playbook.rs`, `src/application/orchestration/policy_engine.rs`, `src/application/orchestration/workflow_status.rs`.
...
- [x] `support/docs/plans/202605081903searched` is a search-result source file, not an execution contract. This board is the execution contract derived from it.
...
- [x] `support/docs/plans/2026-05-05-execution-tree-factor-auto-quant-todo.md` remains authoritative for factor discovery, market/timeframe breadth, trade-density proof, and external strategy iteration.
- [x] `support/docs/plans/2026-05-07-auto-quant-post-factor-runtime-closure-todo.md` remains authoritative for post-factor import, prior-init, real-trade ingestion, structural path export, ranker registration/application, and execution-tree before/after evidence.
- [x] New research imports should begin as additive helpers and artifact contracts under `support/scripts/research/` or `support/scripts/auto_quant_external/` before reopening Rust runtime code.
...
- `support/docs/structural-belief-learning-repo-map.md` confirms delayed reward, IPS, SNIPS, DR, hazard, duration, censoring, and competing-risk surfaces are already partially landed.
...
- `support/scripts/research/hmm_numeric_trainer.py`, `support/scripts/auto_quant_external/external_regime_hmm_viterbi.py`, and `support/scripts/auto_quant_external/external_regime_changepoint_labels.py` already provide external HMM / changepoint hooks.
...
- `support/docs/plans/2026-05-05-execution-tree-factor-auto-quant-todo.md` already defines the breadth / trade-density / resonance / provider-budget rules for factor iteration.
...
- `support/docs/plans/2026-05-02-catboost-path-ranking-target-design.md` and `support/docs/structural-belief-learning-repo-map.md` already define the path-ranking target/export/status contract.
- `support/scripts/auto_quant_external/pandas_path_ranker_trainer.py` and `support/scripts/auto_quant_external/path_ranker_integration.py` already prove external ranker hot-plug and repo-native artifact registration paths.
...
- [x] Read and normalize `support/docs/plans/202605081903searched` into repo-bounded concerns instead of treating its upstream tools as direct architecture.
...
- [x] Workstream 1 / Slice 1 completed as an external-only helper:
  - Added `support/scripts/research/market_data_resolver.py`.
  - Added `support/scripts/research/tests/test_market_data_resolver.py`.
...
- [x] Workstream 2 / Slice 1 completed as an external-only report helper:
  - Added `support/scripts/research/policy_truth_reports.py`.
  - Added `support/scripts/research/tests/test_policy_truth_reports.py`.
...
- [x] Workstream 3 / Slice 1 completed as an external-only factor-candidate helper:
  - Added `support/scripts/research/factor_candidate_pack.py`.
  - Added `support/scripts/research/tests/test_factor_candidate_pack.py`.
...
- [x] Workstream 1 / Slice 2 completed as a minimal runtime readback:
  ...
  - No runtime dependency on `support/scripts/research/market_data_resolver.py` was introduced.
...
- `support/docs/plans/202605081903searched` correctly identifies manual dataset selection as a real usability gap.
...
- future additive helper under `support/scripts/research/` such as `market_data_resolver.py`
...
- `support/scripts/research/market_data_resolver.py` now materializes the Bundle A artifact family directly from:
  - `config/market_data_harness_presets.json`
  - `config/market_relationships.json`
  - optional `support/examples/provider_profiles/*.json`
...
- `python3 -m unittest support.scripts.research.tests.test_market_data_resolver`
...
- `python3 support/scripts/research/market_data_resolver.py --repo-root . --market NQ --profile thrill3r_nq_closed_loop_v1 --output-dir /tmp/ict-engine-bundle-a --timeframe 15m --timeframe 1h --bar-count 1200`
...
- `support/scripts/research/hmm_numeric_trainer.py`
- `support/scripts/auto_quant_external/external_regime_hmm_viterbi.py`
- `support/scripts/auto_quant_external/external_regime_changepoint_labels.py`
- future additive helpers under `support/scripts/research/`
...
- `support/scripts/research/policy_truth_reports.py` now packages the existing repo surfaces into reviewable artifacts:
...
- `support/scripts/research/factor_candidate_pack.py` now packages Auto-Quant manifest evidence into the white-box candidate pack:
...
- `python3 -m py_compile support/scripts/research/hmm_numeric_trainer.py`
- `python3 -m py_compile support/scripts/auto_quant_external/external_regime_hmm_viterbi.py support/scripts/auto_quant_external/external_regime_changepoint_labels.py`
- `./target/debug/ict-engine policy-training-status --symbol DEMO --state-dir /tmp/ict-engine-bundle-b --human`

**Latest evidence**

- `python3 -m unittest support.scripts.research.tests.test_policy_truth_reports`
- `python3 -m py_compile support/scripts/research/policy_truth_reports.py support/scripts/research/tests/test_policy_truth_reports.py`
...
- `python3 support/scripts/research/policy_truth_reports.py --symbol DEMO --policy-status-json /tmp/ict-engine-bundle-b/input/policy_status.json --validation-summary-json /tmp/ict-engine-bundle-b/input/validation_summary.json --temporal-summary-json /tmp/ict-engine-bundle-b/input/temporal_summary.json --recommended-path-json /tmp/ict-engine-bundle-b/input/recommended_path.json --output-dir /tmp/ict-engine-bundle-b/reports`
- `python3 -m unittest support.scripts.research.tests.test_factor_candidate_pack`
- `python3 -m py_compile support/scripts/research/factor_candidate_pack.py support/scripts/research/tests/test_factor_candidate_pack.py`
- `python3 support/scripts/research/factor_candidate_pack.py --manifest-json /tmp/ict-engine-bundle-c/strategy_library.json --strategy-name TrendPullbackDense15m --candidate-spec-json /tmp/ict-engine-bundle-c/candidate_spec.json --autoresearch-status-json /tmp/ict-engine-bundle-c/autoresearch_status.json --output-dir /tmp/ict-engine-bundle-c/out`
...
- `support/scripts/auto_quant_external/path_ranker_integration.py`
- `support/scripts/research/path_rule_trainer.py`
- future additive white-box helper under `support/scripts/auto_quant_external/` or `support/scripts/research/`
...
- `cargo run -- factor-research --symbol DEMO --data support/examples/demo/demo-15m.json --state-dir /tmp/ict-engine-bundle-c --backend auto-quant --human`
...
1. Read `support/docs/plans/202605081903searched` only as source material, not as the implementation contract.
