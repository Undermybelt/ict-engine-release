...
HMM + BBN/loopybayesnet + CatBoost voting + PD Array 检测 + MECE regimes + artifact ledger + reflection_bundle + hard gate
...
集成点：CatBoost voting 之后 → reflection_bundle 生成之前 → workflow_snapshot 写盘之前
...
support/scripts/mece_label_bootstrap.py（仅离线辅助，非真相源）
```
...
Execution Score、OU/Ising/Pythagorean 特征全部喂给 CatBoost voting；SHAP 解释强制绑定到 reflection_bundle；`--execution-focus` 升为默认面。
