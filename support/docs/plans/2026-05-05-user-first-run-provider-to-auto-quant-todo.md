...
> **For Hermes/Codex:** this is an execution TODO for a real user-style trial. Do not change code in this round. Work from the user/agent/operator surface only. Log every bug, blocker, redundancy, misleading prompt, or missing guidance into `support/docs/bug/0.1.0/2026-05-05-user-first-run-provider-to-auto-quant-bug-log.md`.
...
**Tech Stack:** Rust CLI, repo docs under `support/docs/`, local optional providers (TradingView MCP, IBKR), public-market providers (Yahoo Finance, Bybit, Binance, Kraken), external managed integration (`Auto-Quant`).
...
- [x] Read only the user/operator docs needed for the trial:
  ...
  - `support/docs/2026-04-26-ibkr-live-data-bridge-plan.md`
  - `support/docs/2026-04-26-multi-exchange-data-source-integration-plan.md`
...
./target/debug/ict-engine factor-research --symbol DEMO --data support/examples/demo/demo-15m.json --state-dir /tmp/ict-engine-user-first-run-demo --backend native --human
```
