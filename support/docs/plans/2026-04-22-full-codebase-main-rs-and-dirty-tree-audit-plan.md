...
Produce a repo-local audit in `support/docs/` for the entire `ict-engine` codebase with extra focus on:
...
1. A written audit in `support/docs/` with findings, severity, reproduction, impact, and concrete fixes.
...
6. Write the final audit to `support/docs/audits/`.
