...
> **For Hermes/Codex:** treat this markdown as the execution contract for turning `support/docs/external/algo20250505.md` into real `ict-engine` implementation slices. Pick exactly one unchecked workstream at a time. Do not blend lanes. Update this same markdown after every landed slice.
...
**Tech Stack:** Rust CLI, repo docs under `support/docs/`, Python research harnesses under `support/scripts/research/`, persisted artifacts under explicit `--state-dir`, existing `factor-research` / `policy-training-status` / `workflow-status` / structural path ranking surfaces.
...
1. Read `support/docs/external/algo20250505.md` before changing code on this lane.
...
- [x] Execute `Workstream 1 / Slice 2`: add a minimal external rule/tree trainer harness under `support/scripts/research/` and prove artifact registration works end-to-end on `/tmp/...`.
  - landed `support/scripts/research/path_rule_trainer.py`
    - stdlib-only
  ...
  - proved the opt-in end-to-end loop on `/tmp/ict-engine-path-ranker` without adding a new public top-level command:
    ...
    - `python3 support/scripts/research/path_rule_trainer.py`
  ...
  - verification:
    - `python3 -m py_compile support/scripts/research/path_rule_trainer.py`
    ...
    - real `/tmp` closure:
      - `./target/debug/ict-engine factor-research --symbol DEMO --data support/examples/demo/demo-15m.json --state-dir /tmp/ict-engine-path-ranker --backend native --human`
...
- [x] Execute `Workstream 2 / Slice 2`: add the first bounded HMM numeric outer-loop trainer harness.
  - landed `support/scripts/research/hmm_numeric_trainer.py`
    - stdlib-only
  ...
  - verification:
    ...
    - `python3 support/scripts/research/hmm_numeric_trainer.py --symbol DEMO --data support/examples/demo/demo-15m.json --state-dir /tmp/ict-engine-hmm-trainer --out /tmp/ict-engine-hmm-trainer-out/hmm_numeric_trainer_artifact.json --bin ./target/debug/ict-engine`
...
- [x] Execute `Workstream 3 / Slice 2`: add constrained BBN candidate import/review wiring without turning runtime into an online learner.
  - landed `support/scripts/research/bbn_structure_search.py`
  ...
  - updated `support/docs/repo-bbn-cpt-loader-notes.md` so reviewed candidates map back into the fixed-DAG loader path rather than a new runtime learner
  - verification:
    - `python3 -m py_compile support/scripts/research/bbn_structure_search.py`
    - sample `/tmp` candidate generation:
      - `python3 support/scripts/research/bbn_structure_search.py --rows-jsonl /tmp/ict-engine-bbn-rows.jsonl --out /tmp/ict-engine-bbn-candidate.json --max-parent-count 3 --backend pgmpy_hc`
...
- `support/docs/external/algo20250505.md`
- `src/policy_training_command.rs`
- `src/application/entry_models/training_export.rs`
- `src/application/orchestration/structural_playbook.rs`
- `src/application/orchestration/workflow_status.rs`
- `support/docs/plans/2026-05-03-repo-action-board.md`
...
- `support/docs/plans/2026-05-05-algo20250505-ict-engine-integration-plan.md`
...
- [x] Add `support/scripts/research/path_rule_trainer.py`.
...
- [x] Add a support/docs/example snippet showing:
  - export target rows
  - run trainer on `/tmp/...`
  - register artifact
  - enable runtime
...
- [x] Add `support/scripts/research/hmm_numeric_trainer.py`.
- [x] The trainer must:
...
```bash
python3 support/scripts/research/hmm_numeric_trainer.py \
  --symbol DEMO \
...
- `support/docs/repo-bbn-cpt-loader-notes.md`
...
- `src/bbn/trading/update.rs`
- `src/bbn/trading/persistence.rs`
- `support/scripts/research/bbn_structure_search.py`
- `support/docs/repo-bbn-cpt-loader-notes.md`
- `support/docs/plans/2026-05-05-algo20250505-ict-engine-integration-plan.md`
...
- [x] Add `support/scripts/research/bbn_structure_search.py` for external candidate generation.
