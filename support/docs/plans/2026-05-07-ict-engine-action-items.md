...
- `support/scripts/auto_quant_external/pandas_path_ranker_trainer.py` — 核心训练器
- `support/scripts/auto_quant_external/path_ranker_integration.py` — 一键集成脚本
- `support/scripts/auto_quant_external/user_weights_template.json` — 热插拔权重模板
...
```bash
# 生成 scores.csv
python support/scripts/auto_quant_external/pandas_path_ranker_trainer.py \
...
- `support/scripts/auto_quant_external/pandas_path_ranker_trainer.py` — 核心训练器
- `support/scripts/auto_quant_external/path_ranker_integration.py` — 一键集成脚本
- `support/scripts/auto_quant_external/user_weights_template.json` — 用户可编辑权重模板（热插拔）
...
- 无污染：不修改仓库代码
- 用户特定数据：VRP V2 相关特征（qqq_hv/nq_vs_200d/vix3m/vvix_over_vix 等）
- 回退路径：无可用 CatBoost 模型时，优先读取显式 `--user-weights`，否则读取 `<model_dir>/user_weights.json`，再退回内建默认权重
...
python3 support/scripts/auto_quant_external/path_ranker_integration.py \
...
**2026-05-08 验证补充**：
- [x] `support/scripts/auto_quant_external/tests/test_path_ranker_hotplug.py` 新增：
...
- [x] `python3 -m unittest support.scripts.auto_quant_external.tests.test_next_slice_helpers support.scripts.auto_quant_external.tests.test_path_ranker_hotplug`
- [x] `python3 support/scripts/auto_quant_external/path_ranker_integration.py --help` 已显示 `--reuse-model-dir` / `--user-weights`
...
- [x] 为后续真正闭环预留了 additive 能力：
  ...
  - `support/scripts/auto_quant_external/structural_feedback_trade_enricher.py` 可把带结构模板的 trade 源转成 ingest-ready JSONL
...
- ~~阻塞：路径排名需要外部训练器~~
- ~~解决：借用开源训练器或自行构建~~
- **已解决**（2026-05-07）：训练器已实现，见 `support/scripts/auto_quant_external/pandas_path_ranker_trainer.py`
...
- 因子迭代权威 board：`support/docs/plans/2026-05-05-execution-tree-factor-auto-quant-todo.md`
- 因子后运行时闭环 board：`support/docs/plans/2026-05-07-auto-quant-post-factor-runtime-closure-todo.md`
...
- 文件：`support/docs/examples/market-state-nq-confidence-profile.json`
...
- 用法：
  ...
  target/debug/ict-engine validate-market-state \
    ...
    --window-size 200 \
    --step-size 200 \
    --config support/docs/examples/market-state-nq-confidence-profile.json
  ```
