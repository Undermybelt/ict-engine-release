...
- Use `support/docs/factor-artifact-naming-contract.md` as the canonical interpretation layer for:
  - `board_record`
  - `reusable_input`
  - `candidate_pack`
  - `temp_state_dir`
...
- [x] Locked the candidate pack minimum artifact family:
  ...
  - `factor_eval_grid_summary.json`
  - `transfer_score.json`
- [x] Extended `support/scripts/research/factor_candidate_pack.py` so `factor_expression.json` now carries `filter_belief_execution_mapping`.
- [x] Added regression coverage in `support/scripts/research/tests/test_factor_candidate_pack.py` for:
  - populated middle-layer mapping
  - empty/default middle-layer mapping
- [x] Extended `support/scripts/research/factor_candidate_pack.py` so the helper can now build a candidate pack directly from reusable `Freqtrade` backtest zip evidence instead of requiring a hand-authored `strategy_library.json` first.
- [x] Added regression coverage in `support/scripts/research/tests/test_factor_candidate_pack.py` for:
...
- [x] Added an opt-in personal evidence profile lane at `support/examples/factor_candidate_profiles/thrill3r-nq-auto-quant-v1.json`.
- [x] Added `support/scripts/research/factor_candidate_resolver.py` plus `support/scripts/research/tests/test_factor_candidate_resolver.py` so the board now has one additive helper that:
...
- [x] Added a canonical naming / evidence-interpretation layer at `support/docs/factor-artifact-naming-contract.md`.
...
- [x] Converted the regime-only placeholder into a real hot-pluggable artifact lane without polluting the generic surface:
  ...
  - `support/examples/factor_candidate_profiles/thrill3r-nq-auto-quant-v1.json` now opt-in injects the concrete local benchmark JSON bundle
  - `support/scripts/research/factor_candidate_resolver.py` now builds either:
    - execution candidate packs from `freqtrade_backtest_zip`
    - or regime artifact bundles from `regime_benchmark_jsons`
  - `support/scripts/research/regime_artifact_bundle.py` now serves as the additive classifier / transition / resonance artifact emitter
...
- [x] Tightened reusable-input validation so hot-pluggable evidence is consumer-safe rather than path-only:
  - `support/scripts/research/factor_candidate_resolver.py` now validates `freqtrade_backtest_zip` readability with zip integrity checks before marking a lane `artifact_ready=true`
  ...
  - regression coverage added in `support/scripts/research/tests/test_factor_candidate_resolver.py` for:
...
- [x] Continued the Family A breadth lane with one more real explicit candidate pack:
  ...
  - verified `python3 support/scripts/research/factor_candidate_resolver.py --repo-root . --profile thrill3r_nq_auto_quant_v1 --build-packs --output-dir /tmp/ict-engine-factor-candidate-registry-profile-20260508-v3`
...
- [x] Continued the Family A breadth lane with the first explicit 5m timeframe-coverage pack:
  ...
  - verified `python3 support/scripts/research/factor_candidate_resolver.py --repo-root . --profile thrill3r_nq_auto_quant_v1 --build-packs --output-dir /private/tmp/ict-engine-factor-candidate-registry-profile-20260508-v5`
...
- [x] Extended the hot-pluggable reusable-input layer beyond `freqtrade_backtest_zip` so old explicit Family A evidence can still be consumed without rerunning the whole historical workspace:
  - `support/scripts/research/factor_candidate_resolver.py` now accepts `strategy_library_json` as another additive reusable input kind
...
- [ ] For each active factor family, write or refresh one candidate-spec JSON that explicitly fills:
  ...
  - `execution_tree_targets`
  - `structural_feedback_required`
- [ ] For each active factor family, rebuild its candidate pack through `support/scripts/research/factor_candidate_pack.py`.
...
- [ ] Hand off to `support/docs/plans/2026-05-07-auto-quant-post-factor-runtime-closure-todo.md` only after a candidate pack is explicit enough to answer all of the following from artifacts, not chat:
...
- [ ] Use `support/docs/plans/2026-05-09-factor-iteration-pre-bayes-bbn-catboost-execution-tree-todo.md` as the sequencing bridge once a candidate leaves pure factor iteration:
...
4. Build the candidate pack with:
  - `python3 support/scripts/research/factor_candidate_pack.py --manifest-json <strategy_library.json> --strategy-name <strategy> --candidate-spec-json <candidate_spec.json> --autoresearch-status-json <autoresearch_status.json> --output-dir /tmp/<candidate-pack>`
  - or `python3 support/scripts/research/factor_candidate_pack.py --freqtrade-backtest-zip <backtest.zip> --strategy-name <strategy> --candidate-spec-json <candidate_spec.json> --output-dir /tmp/<candidate-pack>`
  - or `python3 support/scripts/research/factor_candidate_resolver.py --repo-root . [--profile <opt-in-profile>] --build-packs --output-dir /tmp/<candidate-pack-root>`
5. Inspect:
  - `factor_expression.json`
  - `factor_eval_grid_summary.json`
  - `transfer_score.json`
