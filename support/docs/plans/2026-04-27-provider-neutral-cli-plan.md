...
**Architecture:** Keep the single-symbol core generic, but make all provider orchestration explicit-request driven. Repo-owned market packs become support/examples/fixtures instead of runtime truth. Public commands stop inferring spot/options/related markets from repo config; callers must pass a request document or explicit symbol/provider overrides. Hardcoded market-family heuristics in the public runtime are either removed or demoted behind optional external policy input.
...
- [2026-04-27-market-data-harness-refactor-plan.md](/Users/thrill3r/projects-ict-engine/ict-engine/support/docs/plans/2026-04-27-market-data-harness-refactor-plan.md)
- [2026-04-27-canonical-market-catalog-debt-closure-plan.md](/Users/thrill3r/projects-ict-engine/ict-engine/support/docs/plans/2026-04-27-canonical-market-catalog-debt-closure-plan.md)
...
- Modify: `src/main.rs`
- Modify: `README.md`
- Modify: `support/scripts/help_audit.py`
...
- Create: `support/examples/provider_requests/`
- Create: `tests/provider_neutral_cli.rs`
- Create: `support/docs/external/provider-request-contract.md`
...
- [ ] Update README examples to prefer `<SYMBOL>`, `<INSTRUMENT>`, `<REQUEST_JSON>`, `<STATE_DIR>`, and sample files under `support/examples/` instead of maintainer-preferred futures symbols.
- [ ] Extend `support/scripts/help_audit.py` so it fails when help output contains repo-owned market exemplars in public-facing flag descriptions.
...
### Task 4: Demote repo market packs to support/examples/fixtures
...
**Files:**
- Modify: `README.md`
- Modify: `support/docs/external/provider-request-contract.md`
- Create: `tests/provider_neutral_cli.rs`
...
- [ ] Manually verify that a sample example request under `support/examples/provider_requests/` still drives the harness successfully.
...
- 为旧路径提供短期明确报错，不做静默兼容
- 对仍需保留的 repo 内部样例统一迁移到 `support/examples/provider_requests/` 和测试 fixture
