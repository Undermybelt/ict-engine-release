...
| done | Source repo macOS CI cargo-cache repair | Owner: Codex current turn, claimed 2026-05-14 after source Actions run `25838723352` failed only macOS `Format`. Root cause evidence: `cargo fmt --check` invoked `rustup-init fmt` after `Swatinem/rust-cache@v2` restored `/Users/runner/.cargo/bin` from key `v0-rust-rust-Darwin-arm64-9d946206`; Ubuntu passed and release mirror passed on an older macOS runner image. Fix: stop caching cargo bin shims, bump the cache prefix, request rustfmt/clippy components explicitly, and add a toolchain sanity step. Verification: source Actions run `25840195711` passed on both `ubuntu-latest` and `macos-latest`, including Toolchain sanity, Format, Docs runtime isolation, Clippy, and Test. |
