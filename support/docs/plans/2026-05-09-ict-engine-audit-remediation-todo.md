...
- [ ] Update `support/docs/plans/2026-05-09-vrp-v2-loop-handoff-todo.md` to stop using one counter for both meanings.
- [ ] Verify:
...
- [ ] Verify:
  - `python3 -m pytest support/scripts/auto_quant_external/tests/test_path_ranker_contract.py -q`
...
- [ ] Measure current `src/main.rs` line count and add it to `support/docs/main-rs-guardrails.md` as baseline debt.
...
- [ ] Inventory all 49 commands and mark output support.
  - Script target: `support/scripts/help_audit.py` can be extended.
- [ ] Create `support/docs/command-output-contract.md` with command matrix.
...
- [ ] Add `support/docs/consumer-quickstart.md` with exactly three flows:
...
- [ ] Add `support/docs/contributor-quickstart.md` with:
  - build/test commands
  - where to add code
...
- [ ] Create `support/scripts/smoke_acceptance.sh`.
...
- [ ] Create `support/scripts/SCRIPTS.md` with groups:
  - public wrappers
  ...
  - local utilities
- [ ] Add `support/scripts/script_manifest.json` with `name`, `stability`, `entrypoint`, `safe_default`, `requires_data`, `test_command`.
- [ ] Extend `support/scripts/help_audit.py` to verify public wrappers default to help/no execution.
...
- [ ] Update `support/docs/factor-catalog.md` to match.
...
- [ ] Add architecture placement rules from `support/docs/main-rs-guardrails.md`.
- [ ] Clarify release repo vs source repo in `support/docs/release-mirror-runbook.md`.
