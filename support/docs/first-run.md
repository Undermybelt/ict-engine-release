...
```bash
cargo run -- factor-research --symbol DEMO --data support/examples/demo/demo-15m.json --backend auto-quant --state-dir /tmp/ict-engine-auto-quant
...
python3 support/scripts/search_cluster.py
python3 support/scripts/evaluate_bottleneck.py
```
...
| "Compare nearby parameters" | `support/scripts/search_local.py --show-config` then `--run --data-root <ict-cleaned-mtf>` |
| "Try a bigger research jump" | `support/scripts/search_cluster.py --show-config` then `--run --data-root <ict-cleaned-mtf>` |
| "Find current blocker" | `support/scripts/evaluate_bottleneck.py --show-config` then `--run --data-root <ict-cleaned-mtf>` and `support/scripts/evidence_quality_breakdown.py` |
| "Summarize existing experiment dirs" | `support/scripts/research_verdict.py <dir...>` |
...
3. `support/scripts/research_verdict.py <state-or-result-dir>`
