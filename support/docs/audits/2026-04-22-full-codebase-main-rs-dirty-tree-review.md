...
```text
ict-engine factor-research --symbol DEMO --data support/examples/demo/demo-15m.json --state-dir <local-path>
```
...
| untracked `support/paper2code/` 57 个文件 | release tarball 和源码边界变重 | 明确是 research sandbox；加 README、requirements policy、smoke tests，或移到 `support/docs/external/` / 独立仓 |
...
`support/paper2code/` 当前有 57 个未跟踪源码/文档文件，`find` 统计约 7,096 行。它包含 `bridge.py`，会动态加载多个 paper module，并依赖 numpy；子目录还带 `rammstein/requirements.txt`、`red_queens_trap/requirements.txt` 等独立运行时。
...
1. 如果这是正式功能，补 `support/paper2code/README.md`、`python -m pytest` 或至少 import smoke，并在 CI 里单独可选跑。
2. 如果只是素材，移动到 `support/docs/external/` 或 `support/paper2code/README.md` 明确“not runtime, research notes only”。
...
已有 `support/scripts/state_cleanup.sh`，但还没有正式 CLI：
...
3. 明确 `support/paper2code/` 的 release 边界。
