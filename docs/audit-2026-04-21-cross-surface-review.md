...
- **建议**:
  ...
  - `scripts/state_cleanup.sh` — 交互式清理（不自动执行）
...
- **建议**:
  - 新建 `docs/environment-variables.md` — 每个变量一行（默认 / 作用 / 示例）
...
- **建议**: 加 `CONTRIBUTING.md` 规范（跑 `cargo test`、跑 `scripts/round2_smoke_replay.sh`、不在 CLI path 上 unwrap、不改 main.rs 超 50 行）
...
- **建议**: 文档化"什么时候用哪个"；agent 协议 spec 一份 `docs/agent-protocol.md`
...
- `scripts/round2_smoke_replay.sh` 加入 CI 作为 smoke gate
...
- 对照 `docs/plans/` 下 20+ 个 plan 文件，核对哪些 plan 的承诺还没落地（plan-reality diff）
- 跑一次 `cargo tree | wc -l` 看依赖总规模
- 审查 `paper2code/` 目录（仓库根里还有一个 Python 子项目，未进 Cargo workspace）
...
> 审计对齐: HEAD = `5a6a050`；参考 `docs/plans/2026-04-21-compare-surface-closure-plan.md`。
...
| 计划 doc | `docs/plans/2026-04-21-compare-surface-closure-plan.md`（389 行） | 步步走的 agentic plan |
...
- **位置**: `docs/plans/2026-04-21-compare-surface-closure-plan.md` 开头
...
- **`paper2code/` 子目录**（Python，未进 Cargo workspace，原审计 §6 已列）
