...
## 定位
- 本文件是对以下两份文档的**可执行派生**：
  - `docs/execution-first-4-sprint-plan.md`（原始 4 Sprint 蓝图）
  - `docs/execution-paper-notes-and-plan-update.md`（论文读后对优先级的修正：Phase A→B→C→D）
- 它不重写原计划，只回答三件事：
  - 当下两份文档与代码之间的差距在哪里？
  - 下一步按什么顺序推进？
  - 每一步的验收条件怎么写？
...
- Sprint 3 MECE
  ...
  - `scripts/mece_label_bootstrap.py`：**缺**（仅作离线辅助）
...
- `scripts/mece_label_bootstrap.py` 只作离线辅助：**不允许**成为系统真相源。
