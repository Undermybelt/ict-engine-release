...
**对 ict-engine 的价值**：
- FCGR 可把 Vec<PDAToken> 变成固定维度 embedding，喂给 CatBoost/XGBoost
...
**产出**：
- `scripts/pda_fcgr_poc.py`
- `docs/plans/dna-sequence-poc-results.md`
- 聚类可视化 + regime 解释
...
**产出**：
- `scripts/pda_transformer_embedding_poc.py`
- PoC 结果报告
- Go/No-Go 判定
...
5. CatBoost/XGBoost voting layer 消费 FCGR embedding（作为额外特征）
...
| Transformer embedding | `PolicyFeatureVector` | CatBoost/XGBoost extra feature |
...
- **价值**: multi-attention 最成熟方案，生成固定维度 embedding 喂给 CatBoost/XGBoost voting layer 或 HMM
...
- `docs/hybrid-regime-clustering-integration-note.md` — hybrid regime 聚类架构
- `docs/ict-factor-mutation-optimization-plan.md` — factor mutation 优化
- `docs/paper-driven-typed-packets-design.md` — typed packet 设计
- `docs/typed-packets-paper-upgrade-plan.md` — packet-first 升级计划
- `docs/experiments/eml-regime-fusion-poc.md` — EML regime fusion PoC（已 rejected，但架构约束可复用）
- `docs/pda_type` — PDA 类型完整定义 + 检测逻辑
- `docs/experiments/ict-execution-setup-tree.md` — ICT 执行决策树
- `docs/research-system-map.md` — 研究系统总览
- `docs/regime-aware` — regime-aware trading 框架
- `docs/execution-paper-notes-and-plan-update.md` — execution-first 架构修正
